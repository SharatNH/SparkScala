...
object MostPopularSuperhero {
  ...
  def parseNames(line: String) : Option[(Int, String)] = {
    var fields = line.split('\"')  //split based on quotation mark
    if (fields.length > 1) {
      
      //1  "heman" say , 1_ heman are in field, so to remove space after id
      // use trim(),removes front and back spaces
